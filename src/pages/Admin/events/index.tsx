...
const EventManagementTable = () => {
  ...
  const [approveModal, setApproveModa1] = useState<boolean>(false);
  // View modal state
  ...
  const confirmReject = async () => {
    try {
      ...
      await axiosInstance.put(
        `/admin/events/${rejectModal.eventId}/reject?remarks=${rejectModal.remarks}`
      );
      setEvents((prev) =>
        prev.map((event) =>
          event.id === rejectModal.eventId
  ...
  const openViewModal = async (id: number) => {
    ...
    try {
      const res = await axiosInstance.get<EventDetail>(
        `/organizer/events/${id}`
      );
                ...
                <tbody className="divide-y divide-slate-200">
                  {loading ? (
                    ...
                    paginatedEvents.map((event) => (
                      ...
                      >
                        <td className="px-6 py-2 text-sm font-medium text-slate-900 w-[10%]">
                          #{event.id}
                        </td>
                        ...
                        <td className="px-6 py-2 w-[15%]">
                          {getPaymentStatusBadge(event.organizerFeeStatus)}
                        </td>
    ...
    <div className="relative w-full max-w-lg bg-white rounded-xl shadow-2xl border border-slate-200">
        ...
        {viewModal.loading ? (
          ...
          <div className="space-y-5">
            ...
            {viewModal.data.imageUrls?.length > 0 && (
              <img
                src={
                  `https://spot.app.codevicesolution.in${viewModal.data.imageUrls[0]}` ||
                  "/placeholder.svg"
                }
                alt={viewModal.data.title}
                className="w-full h-46 object-cover rounded-lg border border-slate-200"
              />
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <div>
                  ...
                  <span className="font-medium text-slate-800">
                    {viewModal.data.venueName}
                  </span>
                </div>
                <div>
                  <span className="text-slate-500">Dates: </span>
                  <span className="font-medium text-slate-800">
                    {formatDate(viewModal.data.startDate)} –{" "}
                    {formatDate(viewModal.data.endDate)}
                  </span>
                </div>
                {/* <div>
              ...
              <div className="space-y-1.5">
                ...
                <div>
                  ...
                  <span className="font-medium text-slate-800">
                    {viewModal.data.seatingType} (
                    {viewModal.data.totalSeats} seats)
                  </span>
                </div>
                <div>
            ...
            {/* <div>
              ...
              <span className="font-medium text-slate-800">
                {viewModal.data.ratings}
              </span>
            </div> */}

            {Array.isArray(viewModal.data.seats) &&
              viewModal.data.seats.length > 0 && (
                <div className="text-xs text-slate-500">
                  Showing {viewModal.data.seats.length} seat entries (not listed
                  individually)
                </div>
              )}
      ...
      <div className="px-5 py-3 border-t border-slate-200 flex justify-end gap-2">
        ...
        {approveModal && (
          <button
            onClick={() => {
              handleApprove(viewModal.data?.id as number);
              setApproveModa1(false);
            }}
            className="px-3 py-1.5 rounded-lg text-white bg-green-600 text-xs font-medium"
          >
            Approve
          </button>
        )}
