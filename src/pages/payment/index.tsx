  ...
  const handleRazorpayPayment = async () => {
    ...
    try {
      const response = await axiosInstance.post(
        `events/organizer-fee/initiate/${eventId}`
      );

      const { orderId } = response.data;

      const options: any = {
        key: "rzp_test_RUxcY5tE583K5C", // replace with your Razorpay test key
        amount: event?.organizerFeeAmount,
        currency: "INR",
        name: "Spot Event Booking",
        description: `Payment for ${event?.title}`,
        order_id: orderId,
        handler: function (responseData: any) {
          setPaymentStatus("processing");

          axiosInstance
            .post(`/events/organizer-fee/verifyPaymentSiganture`, {
              provider: "RAZORPAY",
            ...
            src={`https://spot.app.codevicesolution.in${event.imageUrls[0]}`}
              ...
              {event?.seatingType === "SEAT_LAYOUT" && (
                <div className="shadow-lg border-0 bg-white/80 backdrop-blur-sm rounded-lg">
                  ...
                  <div className="p-6">{renderSeatLayout()}</div>
                </div>
              )}
            </div>
